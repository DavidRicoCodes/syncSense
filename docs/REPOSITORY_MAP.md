...
El paquete padre implementa validación de contratos, procesos locales, SSH con capacidades explícitas, estado atómico, supervisión foreground, NFSv4 explícito, publicación, recuperación e inferencia dummy. `nosync_passive` añade publicación raw con eventos USRP locales y asociaciones `nearest-ntp` derivadas e independientes. `wifi_bf_like` integra el BF definitivo HE NDP-like de 40 MHz como smoke de una sola banda con schema `[8,242]`, sin convertirlo todavía en un experimento científico multibanda activo. PC5 mantiene `runs/<run_id>` como layout canónico y crea para cada intento nuevo un catálogo secundario navegable por metadata experimental; las entradas son enlaces relativos y no duplican datasets. Todos los perfiles hardware habilitan DSP/RF solo con sus flags de autorización; las pruebas automáticas siguen usando dobles. Los workers distribuidos viven en `tools/` y se ejecutan desde clones Git sin instalar el paquete en los clientes. El despliegue acepta ramas de desarrollo o `detached HEAD` siempre que todos los nodos participantes estén limpios y en el mismo commit exacto que PC5.
...
- Módulos BF-like, Doppler, RTC-SI y utilidades de fusión recuperados del trabajo de laboratorio. El sounding HE NDP-like de 40 MHz es el BF definitivo y dispone de un smoke del framework; el Golay52 queda como perfil histórico independiente. Doppler, RTC-SI, fusión y el experimento científico activo completo siguen fuera del recorrido y no sustituyen el modelo externo.
